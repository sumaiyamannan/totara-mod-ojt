...
define(['jquery', 'core/log', 'core/str', 'core/templates', 'core/notification'], function($, log, str, templates, notification) {
     ...
     * @param {Object} state State variables for this autocomplete element.
     * @param {JQuery} originalSelect The JQuery object matching the hidden select list.
     * @param {boolean} markDirty Should the form be marked dirty after this update?
     */
    var updateSelectionList = function(options, state, originalSelect, markDirty) {
        ...
        if (typeof M.core_formchangechecker != 'undefined' && markDirty) {
            M.core_formchangechecker.set_form_changed({target: originalSelect});
        }
    ...
    return /** @alias module:core/form-autocomplete */ {
        ...
        enhance: function(selector, tags, ajax, placeholder, caseSensitive, showSuggestions) {
            ...
            context.items = [];
            context.class = originalSelect.attr('class');
            ...
            $.when(renderInput, renderDatalist, renderSelection).done(function(input, suggestions, selection) {
                ...
                updateSelectionList(options, state, originalSelect, false);
            });
        }
    };
});
