...
define([ 'core/mustache',
    ...
    /** @var {$.Deferred} templateCache - Cache of already loaded templates */
    var templateCache = {};
    ...
    var getTemplate = function(templateName, async) {
        ...
        if (searchKey in templateCache) {
            return templateCache[searchKey].promise();
        }

        // This hasn't been requested in this page cycle
        templateCache[searchKey] = $.Deferred();

        // Now try local storage.
        var cached = storage.get('core_template/' + searchKey);

        if (cached) {
            templateCache[searchKey].resolve(cached);
            return templateCache[searchKey].promise();
        }
        ...
        promises[0].done(
            function (templateSource) {
                ...
                templateCache[searchKey].resolve(templateSource);
            }
        ).fail(
            function (ex) {
                templateCache[searchKey].reject(ex);
            }
        );
        return templateCache[searchKey].promise();
    };
