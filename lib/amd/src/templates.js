...
define([ 'core/mustache',
    ...
    var doRender = function(templateSource, context, themeName) {
        ...
        partialsComplete.done(function () {
            /**
             * replaces the partial placeholder with the actual content of the partial
             *
             * @method replacePartial
             * @param {string} match Which partial to replace (eg. <_p0>)
             */
            var replacePartial = function (match) {
                var partialNumber = match.match(/\d+/)[0];
                promises[partialNumber].done(function (html, templatejs) {
                    js = js + ';\n' + templatejs;
            ...
            $.when.apply($, promises).then(function () {
                ...
                if (requiredStrings.length > 0) {
                    str.get_strings(requiredStrings).done(function(strings) {
                        ...
                        /**
                         * replaces the string placeholder with the actual string
                         *
                         * @method replaceString
                         * @param {string} match Which string to replace (eg. [[_s0]])
                         */
                        var replaceString = function(match) {
                            var stringNumber = match.match(/\d+/)[0];
                            return strings[stringNumber];
                        };

                        // Looped as strings may have partials and partials may have strings ...
                        while (js.indexOf('[[_s') !== -1 || result.indexOf('[[_s') !== -1 || result.indexOf('<_p')  !== -1) {
                            js = js.replace(/\[\[_s\d+\]\]/g, replaceString);
                            result = result.replace(/\[\[_s\d+\]\]/g, replaceString);
                            result.replace(/\<_p\d+\>/g, replacePartial);
                        }
                        stringsLoaded.resolve();
                    });
                } else {
                    // Looped as partials may have partials inside them
                    while (result.indexOf('<_p')  !== -1) {
                        result.replace(/\<_p\d+\>/g, replacePartial);
                    }
                    stringsLoaded.resolve();
                }
