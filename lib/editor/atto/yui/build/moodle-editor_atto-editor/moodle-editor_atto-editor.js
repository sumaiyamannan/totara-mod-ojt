...
Y.extend(Editor, Y.Base, {
    ...
    initializer: function() {
        ...
        // Totara: do not allow editing if text area is marked as readOnly or disabled (used by frozen Totara form elements).
        var readonly = (document.getElementById(this.get('elementid')).readOnly || document.getElementById(this.get('elementid')).disabled);
        if (readonly) {
            template = Y.Handlebars.compile('<div id="{{elementid}}editable" role="textbox" class="{{CSS.CONTENT}}" />');
        }
...
EditorClean.prototype = {
    ...
    _cleanPasteHTML: function(content) {
        ...
        var rules = [
            ...
            // Remove all garbage herfore opening html tag
            {regex: /^([\s\S]+)<html[^>]*>/gi, replace: ""},
            // Remove the head block
            {regex: /<head[^>]*>[\s\S]*?<\/head>/gi, replace: ""},
            ...
            {regex: /<script[^>]*\/>/gi, replace: ""},
            // Remove any iframe blocks
            {regex: /<iframe[^>]*>[\s\S]*?<\/iframe>/gi, replace: ""}
        ];
