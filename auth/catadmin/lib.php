...
/**
 * This helper function gets the active IdP configured.
 * @return array Active IdP;
 */
function auth_catadmin_get_idps($active = false, $asarray = false) {
    global $DB;
    ...
    $idpentitiesrs = $DB->get_records('auth_catadmin_idps', $conditions);
    $idpentities = array();
 ...
 */
function auth_catadmin_get_sp_metadata() {

    global $catadminsaml, $CFG;

    $sourceId = $catadminsaml->spname;
    ...
    foreach ($assertionsconsumerservices as $services) {
        ...
        switch ($services) {
            ...
            case 'urn:oasis:names:tc:SAML:1.0:profiles:browser-post':
                ...
                $acsArray['Location'] = "$CFG->wwwroot/auth/catadmin/sp/saml1-acs.php/{$sourceId}";
                break;
...
function auth_catadmin_get_default_idp() {
    global $DB;

    $defaultidps = $DB->get_records('auth_catadmin_idps', array('activeidp' => 1, 'defaultidp' => 1));
