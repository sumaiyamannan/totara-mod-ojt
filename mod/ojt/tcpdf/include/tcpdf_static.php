...
//-------------------------------------------------------------------------------------------------------
                                ...
                                if (!empty($CFG->proxyhost)) {
                                    ...
                                    if (!empty($CFG->proxytype) && ($CFG->proxytype == 'SOCKS5')) {
                                        curl_setopt($cs, CURLOPT_PROXYTYPE, CURLPROXY_SOCKS5);
                                    }

                                    curl_setopt($cs, CURLOPT_HTTPPROXYTUNNEL, false);

                                    if (empty($CFG->proxyport)) {
                                        curl_setopt($cs, CURLOPT_PROXY, $CFG->proxyhost);
                                    } else {
                                        curl_setopt($cs, CURLOPT_PROXY, $CFG->proxyhost.':'.$CFG->proxyport);
                                    }

                                    if (!empty($CFG->proxyuser) && !empty($CFG->proxypassword)) {
                                        curl_setopt($cs, CURLOPT_PROXYUSERPWD, $CFG->proxyuser.':'.$CFG->proxypassword);
                                        curl_setopt($cs, CURLOPT_PROXYAUTH, CURLAUTH_BASIC | CURLAUTH_NTLM);
                                    }
                                }
//-------------------------------------------------------------------------------------------------------
//----------------------------END REHS-835 HACK----------------------------------------------------------
