...
function mod_scorm_monitorForBeaconRequirement(target) {
    ...
    if (!navigator || !navigator.sendBeacon) {
        ...
        // This is done after adding the mod_scorm_monitoring_for_beacon_requirement flag.
        return;
    }
    ...
    /**
     * Turns off the use of the BeaconA PI.
     */
    ...
    var observe = function(on, callback) {
        if (target.addEventListener) {
            target.addEventListener(on, callback);
        } else if (target.attachEvent) {
            target.attachEvent(on, callback);
        } else if (console && console.log) {
            console.log('mod_scorm: Unable to attach page dismissal event listeners');
        }
    };
    ...
    // This wouldn't work in IE9, however no version of IE gets this far because of the sendBeacon API check.
